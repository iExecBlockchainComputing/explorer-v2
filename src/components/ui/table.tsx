...
function Table({ className, ...props }: React.ComponentProps<'table'>) {
  return (
    <div
      ...
      className="border-grey-600 relative w-full overflow-x-auto rounded-3xl border"
    >
      <table
        data-slot="table"
...
function TableCell({ className, ...props }: React.ComponentProps<'td'>) {
  return (
    <td
      ...
      className={cn(
        'align-middle whitespace-nowrap *:block *:px-2 *:py-6 first:*:pl-4 last:*:pr-4 *:sm:px-3 *:sm:first:pl-6 sm:last:*:pr-6 *:md:px-5 md:first:*:pl-10 md:last:*:pr-10 [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]',
        className
      )}
      {...props}
    />
