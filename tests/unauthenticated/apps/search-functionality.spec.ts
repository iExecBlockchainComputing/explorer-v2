...
test.describe('Apps Search Functionality', () => {
  test('Test search functionality', async ({ page }) => {
    ...
    // Get an app ID from the first row for testing
    const firstRow = page.getByRole('row').filter({ hasNot: page.getByRole('columnheader') }).first();
    await expect(firstRow).toBeVisible();
    
    const appIdCell = firstRow.locator('td').filter({ hasText: /^0x[a-fA-F0-9]/ }).first();
    const fullAppId = await appIdCell.textContent();
    
    if (fullAppId) {
      // Use a portion of the ID for search
      const searchTerm = fullAppId.slice(0, 10); // First 10 characters
